...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    ArrayList<String> nearLines = new ArrayList<String>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
           ...
           // randWordNum2 = randWordNum;
            String realWord = savedInstanceState.getString("key4", wordOrignal);
            wordOrignal = realWord;

            String realHint = savedInstanceState.getString(KEY_TEXT_VALUE2, hint2);
            hint2 = realHint;
        ...
        //else pick random word (and hint)
            ...
            */
            ...
            // ***********EXPERIMENTAL PART FOR GETTING RANDOM WORD!!! ::::
            InputStreamReader inputStream = new InputStreamReader
                    (getResources().openRawResource(R.raw.english_dict));
            ...
            theLine="";
            int lineCtr = 0;
            // System.out.println("THE First LINE NUMBER: " +desiredLine);

            desiredLine = r.nextInt(numLines);
            nearLines = GetWord(theLine, desiredLine, numLines);

            // ***debug
            for (int i = 0; i < nearLines.size(); i++) {
                System.out.println("THE LINE NUMBER ARRAY:" + nearLines.get(i));
            }
            // ***debug

            // look for within the "scanned visible returned array" for  non-blank lines
            for (int j = 0; j < nearLines.size(); j++) {
                String lineChosen = nearLines.get(j);
                if (  ( lineChosen.length() > 3) && (!(lineChosen.isEmpty()))   )    {
                    theLine = lineChosen;
                    break;
                }
            }
            ...
            System.out.println("THE LINE NUMBER: " +desiredLine + " .... " + "THE LINE: " + theLine);

            theLine = theLine.replaceAll("\\(.*?\\) ?", "");
            String wordTemp1 =  theLine.split("\\.")[0];

            String realWordTemp1 = wordTemp1.split("\\s+")[0];

            System.out.println(wordTemp1);      // more splitting stuff
            System.out.println(realWordTemp1);          // hopefully, the word


            String[] hintSplitArr = theLine.split("\\.");    // no iddea how many parts, so just loop through and concatenate them
            hintConcat = "Hint: ";
            for (int z = 1; z < hintSplitArr.length; z++){
                hintConcat += hintSplitArr[z] + "; ";
            }
            //System.out.println(hintArr[1]);       //the hint

            System.out.println(hintConcat);         //the final hint
            // ***********EXPERIMENTAL PART FOR GETTING RANDOM WORD!!! ::::

            //  finally set word to randomized word
            wordOrignal = realWordTemp1;
    ...
    public ArrayList<String> GetWord(String theLine, int desiredLine, int numLines) {
        int lineCtr = 0;
        ArrayList<String> nearByLines = new ArrayList<String>();
        ...
        while (theLine != null) {
            ...
            if (lineCtr >= desiredLine - 6) {

                // check if the line is none sense, then restart loop to try again
                nearByLines.add(theLine);
                theLine = "cat";

                if (lineCtr == desiredLine) {
                    System.out.println(" AT DESIRED POINT!!! ");
                    return nearByLines;
                }
    ...
    protected void onSaveInstanceState (Bundle outState) {
        ...
        outState.putString("key4", wordOrignal);     // hint text
